...
主提交 `67469ae` 的原始意图是移除天气功能及相关依赖，删除天气组件、主页引用和样式，移除高德与教书先生天气 API 调用及 `VITE_WEATHER_KEY` 配置，并更新 README 的功能列表和 API 清单。前置提交 `73dab32` 已将 `Weather.vue` 中的字符串抛错改为 `throw new Error()`，并删除 `console.log`，相关信息记录在变更日志中。
